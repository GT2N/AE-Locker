...
// Truncate a filename for bar prefix display. Keeps the trailing 37 chars when
// the name exceeds 40 chars so the file extension stays visible.
...
struct ProgressTracker::Impl {
    // ProgressBar is non-movable (holds a std::mutex), so we store instances in a
    // deque: deque does NOT relocate its elements on push at either end, which
    // keeps the references registered with DynamicProgress valid for the
    // tracker's lifetime.
    std::deque<indicators::ProgressBar> bar_storage;

    // DynamicProgress stores reference_wrapper<ProgressBar> into `bar_storage`.
    // slot 0  -> overall bar
    // slot i  -> per-file bar for file_idx = i - 1
    indicators::DynamicProgress<indicators::ProgressBar> bars;

    size_t total_files = 0;
    std::atomic<size_t> files_done{0};

    // Index 0 is unused (overall bar slot). Index i+1 is total_chunks for file_idx i.
    // Populated entirely during start() (main thread) and only read afterwards.
    std::vector<size_t> total_chunks_per_file;

    bool started = false;
    bool finished = false;
    std::mutex start_mutex;  // guards start_file register-piece race vs. worker reads
};
...
void ProgressTracker::start(size_t total_files) {
    ...
    // Hide the cursor exactly once for the tracker's lifetime.
    ...
    pimpl_->bar_storage.emplace_back(
        ...
        indicators::option::PrefixText{"Overall: "},
    ...
    for (size_t i = 0; i < total_files; ++i) {
        pimpl_->bar_storage.emplace_back(
            ...
            indicators::option::End{""},
            indicators::option::PrefixText{"[pending] "},
